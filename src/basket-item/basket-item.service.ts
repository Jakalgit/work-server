...
export class BasketItemService {
  ...
  async create(dto: CreateBasketItemDto) {
    const basketItem = await this.basketItemRepository.findOne({where: {itemId: dto.itemId, userId: dto.userId}})
    const item = await this.itemRepository.findOne({where: {id: dto.itemId}})

    if (basketItem) {
      throw new HttpException('Такой товар уже есть в корзине у данного пользователя', HttpStatus.BAD_REQUEST)
    }
    ...
    if (!item.visibility) {
      throw new HttpException('Товар с таким id недоступен для покупки', HttpStatus.NOT_FOUND)
    }
  ...
  async incrementCount(id: number) {
    ...
    if (!basketItem) {
      throw new HttpException('Товара в корзине с таким id не существует', HttpStatus.NOT_FOUND)
    }
    ...
    if (basketItem.count < 99) {
      ...
      if (item.count > basketItem.count) {
        ...
        throw new HttpException('Большего количества нет в наличии', HttpStatus.METHOD_NOT_ALLOWED)
      }
    } else {
      throw new HttpException('Превышение лимита', HttpStatus.METHOD_NOT_ALLOWED)
    }
  }
  ...
  async getAllByUserId(id: number) {
    const basketItems = await this.basketItemRepository.findAll({where: {userId: id}})
    ...
    return await this.itemService.getItemByIds(itemIds)
  }
