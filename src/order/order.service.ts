  ...
  async create(dto: CreateOrderDto) {
    ...
    if (items.length !== itemIds.length) {
      throw new HttpException(
        'Один или нескольо товаров в корзине недоступны для заказа',
        HttpStatus.BAD_REQUEST,
      );
    }

    if (
      (dto.typeDelivery === 1 || dto.typeDelivery === 2) &&
      dto.typePay !== 1
    ) {
      throw new HttpException(
        'Недопустимый способ оплаты для выбранного типа получения заказа',
        HttpStatus.BAD_REQUEST,
      );
    }

    const basketItems = await this.basketItemRepository.findAll({
      where: { id: { [Op.or]: basketItemIds } },
    });
    items.forEach((item) => {
    ...
    const orderItems =
      await this.orderItemRepository.bulkCreate(orderItemsForCreate);
