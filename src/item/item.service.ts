...
export class ItemService {
  ...
  async create(dto: CreateItemDto, imageFiles: any[]) {
    const candidate = await this.itemRepository.findOne({rejectOnEmpty: undefined, where: {article: dto.article}})
    const tagIds: number[] = JSON.parse(dto.tagIds)

    if (candidate) {
      throw new HttpException('Товар с таким артиклем уже существует', HttpStatus.NOT_FOUND)
    }
    ...
    if (!colorCandidate) {
      throw new HttpException('Группы цветов с таким id не существует', HttpStatus.NOT_FOUND)
    }

    let dtoItemTags = []

    if (tagIds.length !== 0) {
      const tags_candidate = await this.tagRepository.findAll({where:
          {id: {[Op.or]: tagIds}}
      })
      if (tags_candidate.length !== tagIds.length) {
        throw new HttpException('Один или несколько жанров не существует', HttpStatus.NOT_FOUND)
      }
      tagIds.forEach(id => {
  ...
  async changeItem(dto: ChangeItemDto) {
    ...
    if (tagIds.length !== 0) {
      ...
      const item_tag_list = await this.itemTagRepository.findAll({where:
          {
            itemId: dto.id
          }
      })
      let tagsForAdd = []
      let itemTagIdsForDelete = []
      tagIds.forEach(id => {
      ...
      if (itemTagIdsForDelete.length !== 0) {
        await this.itemTagRepository.destroy({where:
            {
              itemId: dto.id,
              tagId: {
  ...
  async getAllByFilterPage(dto: GetItemFilterDto) {
    ...
    if (dto.discount) {
      const discounts = await this.discountRepository.findAll({where: {id: {[Op.or]: itemIds}}})
      itemIds = this.checkIdInArray(discounts, itemIds)
    }
    if (dto.popular) {
      const populars = await this.popularRepository.findAll({where: {id: {[Op.or]: itemIds}}})
      itemIds = this.checkIdInArray(populars, itemIds)
    }
    if (dto.novelty) {
      const novelties = await this.noveltyRepository.findAll({where: {id: {[Op.or]: itemIds}}})
  ...
  async getPrices() {
    const items = await this.itemRepository.findAll({where: {visibility: true}})
    let min = 0, max = 0
    if (items.length !== 0) {
  ...
  async addDiscount(id: number, price: number) {
    ...
    discount = await this.discountRepository.create({itemId: id, old_price: item.price})
  ...
  private async addPreviewToItems(items: any[]) {
    ...
    for (let i = 0; i < items.length; i++) {
      const image = images.at(images.findIndex(el => el.itemId === items[i].id))
